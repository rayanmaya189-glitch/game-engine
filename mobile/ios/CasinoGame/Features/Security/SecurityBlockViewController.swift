...
/**
 * Security Block View Controller
 * Displayed when security issues are detected (jailbreak, remote access apps, etc.)
 */
class SecurityBlockViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        // Add uninstall label (only show if remote apps detected)
        if !securityResult.remoteApps.isEmpty {
            ...
            // Add contact label without instruction
            contentView.addSubview(contactLabel)
            contactLabel.translatesAutoresizingMaskIntoConstraints = false
