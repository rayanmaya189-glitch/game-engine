...
// SetDepositLimit sets deposit limits
func (h *UserHandler) SetDepositLimit(ctx context.Context, req *userv1.SetDepositLimitRequest) (*userv1.SetDepositLimitResponse, error) {
	userID := ctx.Value("user_id")
	if userID == nil {
		return nil, ErrUnauthorized
	}

	limits, err := h.userService.SetPlayerLimits(ctx, userID.(string), "DEPOSIT", req.Amount.GetAmount(), req.Period)
	if err != nil {
		return nil, err
	}

	return &userv1.SetDepositLimitResponse{
		Success: true,
		Message: "Deposit limit set successfully",
		Limit:   limits.ToProto(),
	}, nil
}

// SetBetLimit sets bet limits
func (h *UserHandler) SetBetLimit(ctx context.Context, req *userv1.SetBetLimitRequest) (*userv1.SetBetLimitResponse, error) {
	userID := ctx.Value("user_id")
	if userID == nil {
		return nil, ErrUnauthorized
	}

	limits, err := h.userService.SetPlayerLimits(ctx, userID.(string), "BET", req.Amount.GetAmount(), req.Period)
	if err != nil {
		return nil, err
	}

	return &userv1.SetBetLimitResponse{
		Success: true,
		Message: "Bet limit set successfully",
		Limit:   limits.ToProto(),
	}, nil
}

// SetLossLimit sets loss limits
func (h *UserHandler) SetLossLimit(ctx context.Context, req *userv1.SetLossLimitRequest) (*userv1.SetLossLimitResponse, error) {
	userID := ctx.Value("user_id")
	if userID == nil {
		return nil, ErrUnauthorized
	}

	limits, err := h.userService.SetPlayerLimits(ctx, userID.(string), "LOSS", req.Amount.GetAmount(), req.Period)
	if err != nil {
		return nil, err
	}

	return &userv1.SetLossLimitResponse{
		Success: true,
		Message: "Loss limit set successfully",
		Limit:   limits.ToProto(),
	}, nil
}

// GetPlayerLimits retrieves player limits
func (h *UserHandler) GetPlayerLimits(ctx context.Context, req *userv1.GetPlayerLimitsRequest) (*userv1.GetPlayerLimitsResponse, error) {
	userID := ctx.Value("user_id")
	if userID == nil {
		return nil, ErrUnauthorized
	}

	limits, err := h.userService.GetPlayerLimits(ctx, userID.(string))
	if err != nil {
		return nil, err
	}

	return &userv1.GetPlayerLimitsResponse{
		DepositLimit:   limits.ToProto(),
		BetLimit:       limits.ToProto(),
		LossLimit:      limits.ToProto(),
		SelfExcluded:   limits.SelfExclusion,
		ExclusionEndDate: timestamppb.New(*limits.ExclusionEndDate),
	}, nil
}

// SelfExclude handles self-exclusion
func (h *UserHandler) SelfExclude(ctx context.Context, req *userv1.SelfExcludeRequest) (*userv1.SelfExcludeResponse, error) {
	userID := ctx.Value("user_id")
	if userID == nil {
		return nil, ErrUnauthorized
	}

	err := h.userService.SelfExclude(ctx, userID.(string), req.Duration.String(), req.Reason)
	if err != nil {
		return nil, err
	}

	return &userv1.SelfExcludeResponse{
		Success:        true,
		Message:         "Self-exclusion activated",
		SelfExcluded:   true,
		ExclusionStart: timestamppb.Now(),
	}, nil
}
